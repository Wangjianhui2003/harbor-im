...
const useChatStore = defineStore("chatStore", {
  ...
  actions: {
    ...
    insertMessage(msgInfo, chatInfo) {
      ...
      if (
        !chat.lastTimeTip ||
        chat.lastTimeTip < msgInfo.sendTime - TIME_TIP_INTERVAL
      ) {
        chat.messages.push({
    ...
    removePrivateChat(friendId) {
      ...
      for (let idx in chats) {
        if (
          chats[idx].type == CHATINFO_TYPE.PRIVATE &&
    ...
    removeGroupChat(groupId) {
      ...
      for (let idx in chats) {
        if (
          chats[idx].type == CHATINFO_TYPE.GROUP &&
  ...
  getters: {
    ...
    findChatByFriendId(state) {
      return (friendId) => {
        let chats = this.findChats;
        return chats.find(
          (chat) => chat.type == "PRIVATE" && chat.targetId == friendId,
        );
      };
    },
    //根据groupId找chat
    findChatByGroupId(state) {
      return (groupId) => {
        let chats = this.findChats;
        return chats.find(
          (chat) => chat.type == "GROUP" && chat.targetId == groupId,
        );
      };
    },
  },
