  ...
  openStream().then((stream) => {
    ...
    webRTC
      ...
      .then((offer) => {
        ...
        call(friend.value.id, mode.value, offer)
          .then(() => {
            //TODO:播放铃声
          })
          .catch((error) => {
...
const onRTCPrivateMsg = (msgInfo) => {
  //关闭状态下直接忽略，除了发起通话
  if (
    msgInfo.type !== MESSAGE_TYPE.RTC_CALL_VIDEO &&
    msgInfo.type !== MESSAGE_TYPE.RTC_CALL_VOICE &&
    isClosed.value
  ) {
    return;
  }
...
<template>
  ...
  <RTCPrivateCallPanel
    ...
    :friend="friend"
    :modeText="modeText"
    @acceptRTCPrivateEvent="onAccept"
    @rejectRTCPrivateEvent="onReject"
  ></RTCPrivateCallPanel>
</template>
