...
import {
  getMaxReadedPrivateMessageId,
  readPrivateMessage,
  sendMessageReq,
} from "../../api/privateMsg.js";
...
const isBanned = computed(() => {
  return (
    (props.chat.type == CHATINFO_TYPE.GROUP && group.isBanned == true) ||
...
const updateFriendInfo = () => {
  if (isFriend.value) {
    ...
    //有备注显示备注
    friendInfo.showNickname = friend.value.remarkNickname
      ? friend.value.remarkNickname
      : friend.value.friendNickname;
...
const chooseHeadImage = (msgInfo) => {
  if (isGroup.value) {
    ...
    return msgInfo.sendId == userStore.userInfo.id
      ? userStore.userInfo.headImageThumb
