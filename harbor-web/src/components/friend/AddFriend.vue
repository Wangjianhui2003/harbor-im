  ...
  >
    ...
    <el-scrollbar class="userinfo-list">
      ...
      <div
        v-for="user in users"
        :key="user.id"
        v-show="userStore.userInfo.id !== user.id"
      >
        <div class="item">
          <head-image>{{ user.username }}</head-image>
          <div class="item-info">
            {{ user.username }} {{ user.nickname }} {{ user.online }}
          </div>
          <el-button
          ...
          <el-button v-show="isFriend(user.id)" class="item-button" disabled
            >已添加</el-button
          >
        </div>
      </div>
