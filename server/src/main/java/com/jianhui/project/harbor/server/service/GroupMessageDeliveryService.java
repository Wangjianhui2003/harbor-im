...
public class GroupMessageDeliveryService {
    ...
    private static final String DELIVERY_KEY_PREFIX = "im:group:delivery:ack";
    ...
    public void deliverAndAwaitAck(IMRecvInfo recvInfo) {
        IMGroupMessageInfo messageInfo = normalize(recvInfo);
        ...
        for (IMUserInfo receiver : recvInfo.getReceivers()) {
            ...
            if (channelCtx == null) {
                if (messageId == null) {
                    log.warn("未找到channel，跳过无需ack的群聊消息投递，接收者:{},终端:{}",
                            receiver.getId(), receiver.getTerminal());
                    continue;
                }
            ...
            if (Boolean.TRUE.equals(groupMessageDeliveryCache.getIfPresent(deliveryKey))) {
                log.debug("跳过已确认的重复群聊消息消费，messageId:{}, receiver:{}, terminal:{}",
                        messageId, receiver.getId(), receiver.getTerminal());
                continue;
            }
    ...
    public void acknowledge(Long messageId, Long userId, Integer terminal) {
        ...
        groupMessageDeliveryCache.put(key, Boolean.TRUE);
    ...
    private void waitForAck(List<PendingDelivery> pendingDeliveries) {
        ...
        for (PendingDelivery pendingDelivery : pendingDeliveries) {
            ...
            if (remainingNanos <= 0) {
                ...
                throw new IllegalStateException("等待群聊消息ack超时");
            }
            try {
                ...
                cleanupPending(pendingDeliveries);
                throw new IllegalStateException("等待群聊消息ack被中断", e);
            } catch (ExecutionException | TimeoutException e) {
                cleanupPending(pendingDeliveries);
                throw new IllegalStateException("等待群聊消息ack失败", e);
            }
        }
    }
    ...
    private IMGroupMessageInfo normalize(IMRecvInfo recvInfo) {
        if (recvInfo.getData() == null) {
            return null;
        }
        if (recvInfo.getData() instanceof IMGroupMessageInfo messageInfo) {
            return messageInfo;
        }
        JSONObject jsonObject = JSON.parseObject(JSON.toJSONString(recvInfo.getData()));
        IMGroupMessageInfo messageInfo = jsonObject.toJavaObject(IMGroupMessageInfo.class);
        if (messageInfo.getReadCount() == null && jsonObject.getInteger("readedCount") != null) {
            messageInfo.setReadCount(jsonObject.getInteger("readedCount"));
        }
